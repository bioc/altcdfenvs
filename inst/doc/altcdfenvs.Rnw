...
With short oligonuleotide arrays, several probes are designed to match
a target transcript. The probes matching the same target transcript
...
the knowledge of expected transcript might have changed.
Unless one includes the latest development in transcripts into
an analysis, the analysis could suffer from what could be nicknamed a
{\it Dorian Gray} effect. The chip itself does not change, which means
...
The package is directly usable with {\it Affymetrix GeneChip} short
oligonucleotide arrays, and can be adapted or extended for other platforms.
...
the option \Rcode{skip} can let one implement a poor man's parallel sequence matching
very easily.
