...
if you cite it consider citing other packages it depends on
as well. 
...
and \Rfunction{validAffyBatch} to make sure that the instance fullfils 
basic requirements for consistency. 
...
Since the graph is relatively small, we can plot it:
<<fig=TRUE>>=
